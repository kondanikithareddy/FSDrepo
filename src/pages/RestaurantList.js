...
//       } else if (dialogMode === 'edit' && currentRestaurant) {
//         const response = await fetch(`http://localhost:5000/api/restaurants/${currentRestaurant._id}`, {
//           method: 'PUT',
//           headers: {
//             'Content-Type': 'application/json',
//           },
...
//         setRestaurants(
//           restaurants.map((restaurant) =>
//             restaurant._id === currentRestaurant._id ? data : restaurant
//           )
//         );
//       }
//       handleDialogClose();
...
//     try {
//       await fetch(`http://localhost:5000/api/restaurants/${restaurantId}`, {
//         method: 'DELETE',
//       });
...
//                   View Menu
//                 </Button>
//                 <Button size="small" variant="outlined" color="secondary" onClick={() => handleDeleteRestaurant(restaurant._id)}>
//                   Delete
//                 </Button>
//                 <Button size="small" variant="outlined" color="secondary" onClick={() => handleDialogOpen('edit', restaurant)}>
//                   Edit
...
const RestaurantList = () => {
  ...
  return (
    <div style={{ padding: '16px', backgroundColor: '#121212' }}>
      <Typography variant="h4" gutterBottom align="center" style={{ color: '#ffffff' }}>
        Select a Restaurant </Typography>
      ...
      <Grid container spacing={4}>
        {restaurants.map((restaurant) => (
          <Grid item xs={12} sm={6} md={4} key={restaurant._id}>
            <Card sx={{ maxWidth: 345, backgroundColor: '#1e1e1e', color: '#ffffff' }}>
              <CardMedia component="img" height="200" image={restaurant.img ? `/assets/${restaurant.img}` : '/assets/default_image.png'} alt={restaurant.name} />
              <CardContent>
                <Typography gutterBottom variant="h5" component="div">
      ...
      <Dialog open={openDialog} onClose={handleDialogClose}>
        <DialogTitle>{dialogMode === 'add' ? 'Add Restaurant' : 'Edit Restaurant'}</DialogTitle>
        <DialogContent>
          <TextField label="Restaurant Name" fullWidth value={newRestaurant.name} onChange={(e) => setNewRestaurant({ ...newRestaurant, name: e.target.value })} style={{ marginBottom: '20px' }} />
          <TextField label="Image URL" fullWidth value={newRestaurant.img} onChange={(e) => setNewRestaurant({ ...newRestaurant, img: e.target.value })} style={{ marginBottom: '20px' }} />
          <TextField label="Menu Link" fullWidth value={newRestaurant.link} onChange={(e) => setNewRestaurant({ ...newRestaurant, link: e.target.value })} />
        </DialogContent>
        <DialogActions>
          <Button onClick={handleDialogClose} color="primary">
            Cancel
